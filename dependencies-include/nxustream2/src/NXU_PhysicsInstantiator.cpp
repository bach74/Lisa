...
namespace   NXU
{
	...
	NxuPhysicsInstantiator::NxuPhysicsInstantiator(NxuPhysicsCollection *coll, NxOgre::World* world)
	{
		mCollection =   coll;
...
//*** Instance actors
		...
		for (NxU32 i = 0; i < count; ++i)
		{
			...
			else
			{
				...
				// added by bach
				actor->globalPose.multiply(pose, actor->globalPose);
				// end
		...
		if (mCollection)
		{
			...
			if (p)
			{
				...
				if (newScene)
				{
					...
					//NxOgre::World* world=mechO::LisaCore::Instance().getSimulation()->getWorld();
					NxOgre::Scenes* scenes = mWorld->getScenes();
					NxOgre::Scene* scene=scenes->get("MainScene");
					assert(scene!=NULL);

					// start - added by bach

					// instance OgreMaterials
					for (NxU32 i=0; i<p->mMaterials.size(); ++i)
					{
						NxU32 nbMaterials=newScene->getNbMaterials();
						NXU::NxMaterialDesc *mat=p->mMaterials[i];
						::NxMaterialDesc desc;
						mat->mInstanceIndex=mat->mMaterialIndex;
						mat->copyTo(desc,cc);
						...
						{
							// create new material
							newScene->createMaterial(desc);
						}
						else
						{
							// default material
							NxMaterial *instance=newScene->getMaterialFromIndex(i);
							instance->loadFromDesc(desc);
						}
					}

					// instance OgreModels
					for (NxU32 i=0; i<p->mActors.size(); ++i)
					{
						// find i-th actor and copy the properties
						NXU::NxActorDesc *actor=p->mActors[i];
						::NxActorDesc desc;
						actor->copyTo(desc,cc);
						::NxBodyDesc body;
						if (actor->mHasBody)
						...
						if (strcmp(actor->name,"(null)"))
						{
							// set position
							NxOgre::Pose pose(actor->globalPose);
							// set actor parameters from body description
							NxOgre::ActorParams actorParams;
							actorParams.fromNxActorDesc(desc);
							NxOgre::NodeRenderableParams nodeRenderableParams;
							nodeRenderableParams.setToDefault();
							nodeRenderableParams.mIdentifier=NxOgre::NxString(actor->name);
							nodeRenderableParams.mGraphicsModel = NxOgre::NxString(actor->name).append(".mesh");
							nodeRenderableParams.mGraphicsModelPose = pose;

							// override material if needed
							// nodeRenderableParams.mGraphicsModelMaterial = "Metal_dark/Metal";
							// override scale if needed
							// set visual model scale, unfortunately this parameter isn't written to a collada file
							// so you should get it from your modeler software
							// nodeRenderableParams.mGraphicsModelScale = NxVec3(0.04,0.04,0.04);

							// must create offset so that we have additional scene node that
							// we can rotate or translate (change nxOgre code - to always create offset node)

							// create body based on the parameters above
							// shapegroup parameter is not needed but it cannot be NULL
							NxOgre::Body* newBody=scene->createBody(actor->name,new NxOgre::CompoundShape(),pose,nodeRenderableParams,actorParams);

							//NxOgre::OgreNodeRenderable* renderable=(NxOgre::OgreNodeRenderable*)newBody->getRenderable();
							// since ogre coordinate system has +x right, +y up and +z from screen,
							// and 3ds has +x right, +y in screen and +z up, we need to transform the objects, so they would look the same
							// in both systems
							// (we don't rotate the object, but the node it's attached to)
							// Ogre::SceneNode* node = renderable->getOffsetNode();
							// node->pitch(Ogre::Degree(90));
							// UPDATE: we don't need to do this anymore since exporter can do this for us

							actor->flags=(NxActorFlag)(actor->flags|NXU::NX_AF_FORCE_CONE_FRICTION);
