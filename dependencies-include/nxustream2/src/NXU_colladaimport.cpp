...
// list of collada elements we even care about.
	enum CELEMENT
	{
		...
		CE_IGNORE,
		// added by bach
		CE_DRIVE_CONTROLLER_TYPE,
		CE_DRIVE_CONTROLLER_PARAMS,
		// end bach
		CE_LAST
	};
...
#define WARN(x) reportWarning(x)
	...
	*/
	...
	enum JointDriveType
	{
		...
		// added by bach
		MY_SERVO_DRIVE, REVOLUTE_DRIVE, NO_MOTOR_DRIVE, FIXED_JOINT
		//end
	};
		...
		public:
			...
			void get(NxJointDriveDesc   &d)const
			{
				switch (driveType)
				{
						// added by Bach
					case MY_SERVO_DRIVE:
						d.driveType =   NX_MY_SERVO_DRIVE;
						break;
					case REVOLUTE_DRIVE:
						d.driveType =   NX_REVOLUTE_DRIVE;
						break;
					case NO_MOTOR_DRIVE:
						d.driveType =   NX_NO_MOTOR_DRIVE;
						break;
					case FIXED_JOINT:
						d.driveType =   NX_FIXED_JOINT;
						break;
						// end
					case JDT_POSITION:
	...
	class   C_RigidConstraint: C_Base
	{
		public:
			C_RigidConstraint(TiXmlElement *element):   C_Base(element)
			{
				...
				gearRatio   =   1;
				// bach
				mDriveControllerParams[0]=0;
				mDriveControllerParams[1]=0;
				mDriveControllerParams[2]=0;
				mDriveControllerType=0;
				//end bach
			}
			...
			virtual void setText(CELEMENT   operation, const char   *data)
			{
				switch (operation)
				{
					case CE_DRIVE_TYPE:
						...
						{
							// added by Bach
							if (_stricmp(data,  "MY_SERVO_DRIVE") ==    0)
							{
								mDrive->driveType   =   MY_SERVO_DRIVE;
							}
							if (_stricmp(data,  "REVOLUTE_DRIVE") ==    0)
							{
								mDrive->driveType   =   REVOLUTE_DRIVE;
							}
							if (_stricmp(data,  "NO_MOTOR_DRIVE") ==    0)
							{
								mDrive->driveType   =   NO_MOTOR_DRIVE;
							}
							if (_stricmp(data,  "FIXED_JOINT") ==    0)
							{
								mDrive->driveType   =   FIXED_JOINT;
							}
							// end
					...
					case CE_DRIVE_POSITION:
						...
						break;
						// added by bach
					case CE_DRIVE_CONTROLLER_PARAMS:
						Asc2Bin(data,   3, "f", &mDriveControllerParams);
						break;
					case CE_DRIVE_CONTROLLER_TYPE:
						(strstr(data,"velocity")==NULL)?mDriveControllerType=1:mDriveControllerType=0;
						break;
						// end bach
			...
			//added by Bach
			void getJointRevolute(NxSceneDesc *scene, C_Query *q, C_PhysicsModel    *pmodel);
			//end
			...
			//added by bach
			float mDriveControllerParams[3];    //P,I,D
			int mDriveControllerType;       // 0-velocity,1-position,2-torque
			//end bach
	};
	...
	class   C_RigidBody: public C_Base
	{
		public:
			...
			virtual void setText(CELEMENT   operation, const char   *svalue)
			{
				switch (operation)
				{
					...
					case CE_DISABLE_COLLISION:
						...
						break;
						//bach
					case CE_ENABLE_PRESSURE_SENSOR:
						mEnablePressureSensor   =   getTF(svalue);
						break;
						//endbach
			...
			void getActor(NxSceneDesc *scene, C_Query *q, const NxMat34 &mat, const NxVec3  &velocity, const NxVec3 &angularVelocity)
			{
				...
				// bach
				if (mEnablePressureSensor)
				{
					a->flags = (NxActorFlag)(a->flags|MY_AF_PRESSURE_SENSOR);
				}
				//endbach
...
// Note..needs to   support overrides!
	...
	void C_InstanceRigidConstraint::getJoints(NxSceneDesc *scene, C_Query *q, C_PhysicsModel    *pmodel)
	{
		...
		if (pmodel &&   mConstraint)
		{
			...
			{
				//bach
				if ((rc->twistDrive.driveType==REVOLUTE_DRIVE)
					||(rc->twistDrive.driveType==MY_SERVO_DRIVE)
					||(rc->twistDrive.driveType==NO_MOTOR_DRIVE)) {
					rc->getJointRevolute(scene, q, pmodel);
				} else {
					// endbach
	...
	// added by bach
	void C_RigidConstraint::getJointRevolute(NxSceneDesc    *scene, C_Query *q, C_PhysicsModel *pmodel)
	{
		NxRevoluteJointDesc *j=new NxRevoluteJointDesc;

		if ((mName!=NULL)&&(strcmp(mName,"(null)")!=0))
		{
		...
		else
		{
			...
			char* temp = new char[strlen(rb1->mActorDesc->name)+strlen(rb2->mActorDesc->name)+4];
			strcpy(temp, rb1->mActorDesc->name);
			strcat(temp, "-");
			strcat(temp, rb2->mActorDesc->name);
			j->name = getGlobalString(temp);
			delete temp;
			temp=NULL;
		}
		...
		if (rb1)
		{
			if (rb1->mActorDesc==NULL) return;
			j->mActor0=getGlobalString(rb1->mActorDesc->mId);
		}
		if (rb2)
		{
			if (rb2->mActorDesc==NULL) return;
		...
		// visualize joint
		j->jointFlags=(NxJointFlag)(j->jointFlags|NX_JF_VISUALIZATION);

		// Joint motors are used to supply a relative torque between two bodies connected by a revolute or spherical joint.

		// The lower body (body2) will start to accelerate around the kinematic upper body (body1)
		// until it comes to a constant rotational velocity.  The torque supplied to box2 is maxForce.
		// The target rotational velocity is velTarget.  The motor is set to freeSpin, so if the motor is
		// turned off during the simulation (maxForce set to 0), body2 will continue spinning.
		// If freeSpin is not set, a braking force will be applied and body2 will eventually come to a halt.
		// If freeSpin is false, a breaking force when vel>target is also applied.

		j->motor.maxForce=twistDrive.forceLimit;
		j->motor.velTarget=driveAngularVelocity.x;
		j->motor.freeSpin=true;
		if ((twistDrive.driveType!=MY_SERVO_DRIVE)&&(twistDrive.driveType!=NO_MOTOR_DRIVE))
			j->flags=(NxRevoluteJointFlag)(j->flags|NX_RJF_MOTOR_ENABLED);

		int type=this->mDriveControllerType;
		float* controllerParams=this->mDriveControllerParams;
		char *s=new char[256];
		sprintf(s,"%f %f %f %d",mDriveControllerParams[0],mDriveControllerParams[1],mDriveControllerParams[2],mDriveControllerType);
		j->mUserProperties=s;


		/*NxJointLimitPairDesc limitDesc;
		limitDesc.high.value=;
		limitdesc.low.value;
		j->limit=limit;*/
		...
		if (mProjectionMode)
		{
			...
			// Bodies constrained by joints are moved iteratively under normal simulation.
			// Because of errors introduced during this iteration, joints may need to occasionally be forced back to within
			// a certain projection distance of each other.  Joint projection is a method to combat the inaccuracy of joint simulation.
			// Ideally, the projection distance would be zero, but this puts a heavy restriction on the dynamic motion of the bodies,
			// and the results can end up looking forced and non-physical.
			// Instead the projection distance should be set to a small permissible joint error value.
			// You can set this value to taste depending on how much dynamic motion you want from the bodies and
			// how much you can tolerate them deviating from their joint constraints at any given time step.

			// You can specify the maximum error you wish to allow in a joint before projection kicks in.
	...
	void C_RigidConstraint::getJoint(NxSceneDesc    *scene, C_Query *q, C_PhysicsModel *pmodel)
	{
		...
		j->twistLimit.low.value = (mAngularMin.z==-FLT_MAX)?0: mAngularMin.z * DEG_TO_RAD;
		...
		j->twistLimit.high.value = (mAngularMax.z==FLT_MAX)?0:mAngularMax.z * DEG_TO_RAD;
		...
		// added by bach - fixed joint support
		// since every joint is loaded as d6 joint, just by changing this flag later it
		// will be instantiated as fixed joint
		if (j->twistDrive.driveType==NX_FIXED_JOINT) {
			j->mType = SC_NxFixedJointDesc;
		}
		// end
