...
SimulationImpl::SimulationImpl(Scene* _scene)
{
	...
	mInputController = new SimulationInputController(this);
	...
	mWorld = new NxOgre::World(worldParams);
	...
	sceneParams.mGravity = NxVec3(0, -9.80665, 0);
	mPhyScene = mWorld->createScene(THE_WORLD, &mScene->getSceneManager(), sceneParams);
	...
	MySceneController* sceneController = new MySceneController(mPhyScene);
	sceneController->setTiming(1.0f/100.0, 250, 0);
...
void SimulationImpl::loadScene(const std::string& filename)
{
	...
	try
	{
		...
		loaded = file->LoadScene(mWorld, filename.c_str());
		...
		if (loaded)
		{
			...
			mTrajectory=new Trajectory(filename);
...
{
	OIS::MouseState ms;
	ms.Z.rel = Config::Instance().getCameraZoomSpeed() * (distance);
...
{
	ActuatorIterator it = ActuatorIterator(mActuators);
...
Link* SimulationImpl::getSelectedObject() const
{
	...
	LinkIterator it = LinkIterator(mLinks);
